...
document.getElementById("runBtn").addEventListener("click", function() {
    ...
    const language = document.getElementById("editorSelect").value;
    const output = document.getElementById("output");
    const errorBox = document.getElementById("errorBox");

    errorBox.innerText = "";  // Pehle errors clear kar do

    let code = editor.value;

    try {
        if (language === "js") {
            // Esprima se JS syntax check karenge
            esprima.parseScript(code);
            // JS code ko HTML template mein daal kar iframe me run karenge
            const htmlTemplate = `<html><body><script>${code}<\/script></body></html>`;
            output.srcdoc = htmlTemplate;
        } else if (language === "html") {
            // Direct HTML output karenge
            output.srcdoc = code;
        } else if (language === "css") {
            // CSS ko ek HTML template ke andar daal kar iframe me run karenge
            const htmlTemplate = `<html><head><style>${code}</style></head><body></body></html>`;
            output.srcdoc = htmlTemplate;
        }
    } catch (e) {
        // Agar error aata hai to error box mein show karenge
